...
// ================= Problem Card =================
function MeasurementCard({ idx, problem, answer, setAnswer, result, showAnswer, onReset, onFirstType }) {
    ...
    <div className={`rounded-3xl border-2 ${border} ${bg} shadow-md p-6 flex flex-col gap-4`}>
      ...
      <div className="flex items-end justify-center gap-4">
        ...
        <div className="text-right">
          ...
          <div className="text-sm text-gray-500">({displayMeters} ม. {displayCm} ซม.)</div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-center gap-2 text-lg">
          <input
            ref={inputRef}
            inputMode="numeric"
            className="w-16 text-center border-2 border-sky-300 rounded-xl py-2 px-2 focus:outline-none focus:ring-2 focus:ring-sky-400"
            placeholder="?"
            value={answer.meters || ""}
            onChange={(e) => {
              const v = e.target.value.replace(/\D/g, "");
              if (!answer.meters && !answer.centimeters) onFirstType?.();
              setAnswer(idx, { ...answer, meters: v });
            }}
          />
          ...
          <input
            ...
            className="w-16 text-center border-2 border-sky-300 rounded-xl py-2 px-2 focus:outline-none focus:ring-2 focus:ring-sky-400"
            placeholder="?"
            value={answer.centimeters || ""}
            onChange={(e) => {
              const v = e.target.value.replace(/\D/g, "");
              if (!answer.meters && !answer.centimeters) onFirstType?.();
              setAnswer(idx, { ...answer, centimeters: v });
            }}
          />
        ...
        <div className="flex items-center justify-center gap-2 text-lg">
          ...
          <input
            inputMode="numeric"
            className="w-20 text-center border-2 border-emerald-300 rounded-xl py-2 px-2 focus:outline-none focus:ring-2 focus:ring-emerald-400"
            placeholder="?"
            value={answer.totalCm || ""}
            onChange={(e) => {
              const v = e.target.value.replace(/\D/g, "");
              if (!answer.meters && !answer.centimeters && !answer.totalCm) onFirstType?.();
              setAnswer(idx, { ...answer, totalCm: v });
            }}
          />
          <span className="font-semibold">เซนติเมตร</span>
        </div>
...
export default function MeasurementApp() {
  ...
  const [answers, setAnswers] = useState(() => problems.map(() => ({ meters: "", centimeters: "", totalCm: "" })));
  ...
  function resetAll() {
    ...
    setProblems(next);
    setAnswers(next.map(() => ({ meters: "", centimeters: "", totalCm: "" })));
  ...
  function onReset(idx) {
    setAnswers((prev) => prev.map((a, i) => (i === idx ? { meters: "", centimeters: "", totalCm: "" } : a)));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 via-white to-teal-50 text-zinc-800">
      ...
      <main className="max-w-6xl mx-auto p-6 pt-3">
        <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            {[6, 9, 12].map((n) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-base font-semibold border-2 ${
                  count === n ? "bg-amber-600 text-white border-amber-600" : "bg-zinc-50 hover:bg-zinc-100"
                }`}
              >
                {n}
              </button>
          ...
          <div className="flex items-center gap-2 bg-white/80 backdrop-blur rounded-2xl px-4 py-3 border-2 border-amber-100 shadow-sm">
            ...
            {[
              { key: "easy", label: "ง่าย (10ซม.)" },
              { key: "medium", label: "ปานกลาง (5ซม.)" },
              { key: "hard", label: "ยาก (1ซม.)" },
            ].map((lv) => (
              <button
                key={lv.key}
                onClick={() => applyLevel(lv.key)}
                className={`px-4 py-2 rounded-full text-base font-semibold border-2 ${
                  level === lv.key ? "bg-teal-600 text-white border-teal-600" : "bg-zinc-50 hover:bg-zinc-100"
                }`}
              >
                {lv.label}
              </button>
