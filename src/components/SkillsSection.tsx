...
const SkillCard: React.FC<{ skill: Skill; onPreview?: (skill: Skill) => void }> = ({ skill, onPreview }) => {
  ...
  return (
    <div
      className={`relative rounded-2xl shadow-lg hover:-translate-y-1 hover:shadow-xl transition-all duration-300 bg-gradient-to-br ${skill.from} ${skill.to} ring-1 ring-black/5 p-6`}
      role="button"
      tabIndex={0}
    >
      {/* Sticker emoji */}
      {skill.sticker && (
        <div className="absolute top-3 right-3 text-2xl animate-bounce" aria-hidden="true">
          {skill.sticker}
        </div>
      )}
      ...
      <Link
        ...
        className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-fuchsia-500 to-indigo-500 hover:from-fuchsia-400 hover:to-indigo-400 text-white text-sm font-medium rounded-full shadow-md ring-1 ring-black/10 hover:-translate-y-0.5 transition-all duration-200"
        aria-label={`ดูตัวอย่างแบบเรียน ${skill.title}`}
      >
...
const SkillsSection: React.FC<SkillsSectionProps> = ({ skills = defaultSkills, onPreview }) => {
  return (
    <section className="mb-12">
      <div className="text-center mb-8">
        ...
        <p className="text-white/80 text-lg max-w-2xl mx-auto">
          เลือกทักษะที่อยากฝึกฝน — เพิ่มสติ๊กเกอร์การ์ตูนน่ารักดึงดูดใจเด็ก ๆ
        </p>
      </div>
