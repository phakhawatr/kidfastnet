-- Create function to check if user is already logged in from another device
CREATE OR REPLACE FUNCTION public.check_user_session_status(user_email text, new_session_id text, new_device_info text DEFAULT NULL)
...
BEGIN
    ...
    IF user_record.is_online = true AND user_record.session_id IS NOT NULL AND user_record.session_id != new_session_id THEN
        ...
        IF user_record.last_activity_at > (now() - interval '5 minutes') THEN
