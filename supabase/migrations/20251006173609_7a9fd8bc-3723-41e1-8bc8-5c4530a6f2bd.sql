...
-- Create function with payment fields included
CREATE FUNCTION public.admin_get_user_registrations(admin_email TEXT)
RETURNS TABLE(
    id uuid, 
    member_id text, 
    nickname text, 
...
BEGIN
  ...
  -- Return all registrations with payment fields
  RETURN QUERY 
  SELECT 
    ur.id, 
    ur.member_id, 
