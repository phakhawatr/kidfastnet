-- Update session conflict message
...
BEGIN
    ...
    IF user_record.is_online = true AND user_record.session_id IS NOT NULL AND user_record.session_id != new_session_id THEN
        ...
        IF user_record.last_activity_at > (now() - interval '30 minutes') THEN
            RETURN QUERY SELECT false, 'ขณะนี้บัญชีนี้ ถูกเข้าใช้งานโดยเครื่องอื่นอยู่ กรุณาตรวจสอบการใช้งานของท่าน'::text, user_record.id, user_record.nickname;
            RETURN;
        ELSE
