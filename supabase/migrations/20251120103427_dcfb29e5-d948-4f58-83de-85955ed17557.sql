...
DROP FUNCTION IF EXISTS public.admin_get_user_registrations(text);

-- Create updated function with is_teacher flag
...
BEGIN
  ...
  -- Return all registrations with is_teacher flag
  RETURN QUERY 
  SELECT 
    ur.id, 
    ur.member_id, 
    ...
    ur.ai_usage_count,
    EXISTS(
      SELECT 1 FROM user_roles usr 
      WHERE usr.user_id = ur.id AND usr.role = 'teacher'
    ) as is_teacher
