...
DROP POLICY IF EXISTS "Users can update their own affiliate code" ON public.affiliate_codes;
DROP POLICY IF EXISTS "Anyone can read affiliate codes for validation" ON public.affiliate_codes;
...
CREATE POLICY "Users can view their own affiliate code v2"
  ON public.affiliate_codes FOR SELECT
  USING (is_user_affiliate_owner(user_id));

CREATE POLICY "Users can insert their own affiliate code v2"
  ON public.affiliate_codes FOR INSERT
  WITH CHECK (is_user_affiliate_owner(user_id));

CREATE POLICY "Users can update their own affiliate code v2"
  ON public.affiliate_codes FOR UPDATE
  USING (is_user_affiliate_owner(user_id));

CREATE POLICY "Users can view their own referrals v2"
  ON public.affiliate_referrals FOR SELECT
  USING (is_user_affiliate_owner(referrer_id));

CREATE POLICY "Users can view their own points v2"
  ON public.user_points FOR SELECT
  USING (is_user_affiliate_owner(user_id));

CREATE POLICY "Users can view their own rewards v2"
  ON public.affiliate_rewards FOR SELECT
  USING (is_user_affiliate_owner(user_id));

-- Make affiliate codes publicly readable for validation during signup
CREATE POLICY "Anyone can read affiliate codes for validation v2"
  ON public.affiliate_codes FOR SELECT
  USING (true);
