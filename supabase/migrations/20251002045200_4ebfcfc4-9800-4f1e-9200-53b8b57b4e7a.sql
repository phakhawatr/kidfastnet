-- Add UNIQUE constraint to affiliate_codes.user_id to prevent duplicates
-- This will allow the generate_affiliate_code function to work correctly
ALTER TABLE public.affiliate_codes
ADD CONSTRAINT affiliate_codes_user_id_unique UNIQUE (user_id);

-- Update the generate_affiliate_code function to handle the unique constraint properly
